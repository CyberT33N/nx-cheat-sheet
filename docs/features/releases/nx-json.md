# nx.json
- https://github.com/nrwl/nx/blob/master/packages/nx/src/config/nx-json.ts

## release
- https://nx.dev/docs/reference/nx-json#release
- https://nx.dev/docs/features/manage-releases


### projects
```json
{
  "release": {
    "projects": ["packages/*"],
  },
}
```



### releaseTagPattern
- https://nx.dev/docs/reference/nx-json#release-tag
- docs\features\releases\tags\nx-json.md







<br><br>




### git
- https://nx.dev/docs/reference/nx-json#git
- docs\features\releases\git\nx-json.md





<br><br>



### groups
- docs\features\releases\groups\nx-json.md













<br><br>




### version
- https://nx.dev/docs/guides/nx-release/automatically-version-with-conventional-commits#enable-automatic-versioning
- docs\features\releases\version\nx-json.md


### conventionalCommits
- https://nx.dev/docs/guides/nx-release/customize-conventional-commit-types
- docs\features\releases\version\conventional-commits\nx-json.md


















<br><br>



### versionPlans
- https://nx.dev/docs/guides/nx-release/file-based-versioning-version-plans
- docs\features\releases\version\version-plans\nx-json.md













<br><br>



### changelog
- https://nx.dev/docs/guides/nx-release/configure-changelog-format
- docs\features\releases\changelog\nx-json.md